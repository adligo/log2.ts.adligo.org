...
export class LogCtx implements I_LogCtx {
  ...
  private log: I_Log;
  private logs: Map<string, I_Log> = new Map();
  ...
  public getLog(logName: string): I_Log {
    if (!this.logs.has(logName)) {
      ...
      console.log("Creating log " + logName + " with level " + level);
      this.logs.set(logName, new Log(logName, level, this.config.getFormat(), this.console));
    }
